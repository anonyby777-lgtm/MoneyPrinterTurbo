---
name: teusflix-movies
description: Use this skill whenever the user wants to create a finished movie-recap short video from a movie title, in the style of the Teusflix channel. This includes Brazilian Portuguese movie summaries ("resumo de filmes"), trailer-style edits, and movie reviews/critiques as 9:16 vertical shorts with narration, subtitles, and background music. Use it when the user provides a movie title, asks for a movie recap/summary video, or references Teusflix. Use this skill when the expected outcome is a final video file, not setup instructions.
...
metadata:
  author: "MoneyPrinterTurbo contributors"
  version: "1.0.0"
  upstream: "https://github.com/harry0703/MoneyPrinterTurbo"
  based_on: "moneyprinterturbo-video"
---

# MoneyPrinterTurbo Movie Shorts (Teusflix style)

The user only needs to provide a movie title. Complete installation, configuration reuse, generation, waiting, and final MP4 delivery automatically. Do not stop after giving instructions or commands.

This skill produces the same Teusflix-style short as the generic `moneyprinterturbo-video` skill, but specialized for movies: Brazilian Portuguese narration, a hook-first "resumo de filme" script, 9:16 portrait, subtitles, and background music.
...
Unless the user requests otherwise, generate one Portuguese (pt-BR) `9:16` portrait short with Pexels footage, the `pt-BR-ThalitaNeural-Female` Edge TTS voice, subtitles, background music, and a hook-first movie recap script. The default style is `resumo` (movie recap); use `--style trailer` for a trailer-like edit and `--style analise` for a review/critique. Install or reuse MoneyPrinterTurbo under the user's home directory.

## Execution

### 1. Locate the helper

Resolve `SKILL_DIR` from this `SKILL.md` file. The helper is the adjacent `movies_agent.py`. Set the terminal tool's working directory to `SKILL_DIR` and invoke the helper by its relative filename. Do not put the absolute helper path in the command, and do not run an extra `ls` or `dir` check.

This is required on Windows because some agent terminal validators remove backslashes from absolute paths embedded in commands. Using `movies_agent.py` with `workdir=SKILL_DIR` avoids that failure and works on both macOS and Windows.
...
```text
https://raw.githubusercontent.com/harry0703/MoneyPrinterTurbo/main/docs/skill/teusflix-movies/movies_agent.py
```
...
```bash
uv run --no-project --python 3.11 python movies_agent.py --movie "<movie title>"
```

For a trailer-style edit or a review/critique, add `--style trailer` or `--style analise`:

```bash
uv run --no-project --python 3.11 python movies_agent.py --movie "<movie title>" --style trailer
```
...
`movies_agent.py` emits `VIDEO_FILE` only after confirming that the file exists and is non-empty. Do not run another `ls`, `stat`, or file validation command.
...
```text
~/MoneyPrinterTurbo/.agent-logs/teusflix-movies/latest-result.json
```
...
Summary: Portuguese (pt-BR) 9:16 movie recap short with voice-over, subtitles, and background music.
```
...
A terminal-tool path validation error is not a video-generation failure because the helper did not start. Correct the working directory and retry the relative command once. Never ask the user to copy `movies_agent.py`, run commands manually, or confirm whether the agent should continue.

## Configuration and Background Fallback

The helper may read the complete local `config.toml` to reuse existing settings, but it must never print its contents. It reuses a working LLM provider automatically and validates configured Pexels keys through the authenticated My Collections endpoint before generation. Credentials configured for the generic `moneyprinterturbo-video` skill are reused as-is.
...
- This skill assumes the movie is a topic for an original narrated recap; it does not download or reuse footage from the movie itself.
